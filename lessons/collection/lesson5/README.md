...
The *resolution* of an ADC chip describes the number of unique readings that it can produce to represent this magnitude. The resolution value is given in *bits* (a bit is a binary 1 or 0). A ten bit ADC, for example, can produce values between 0 and 1023 (`1111111111` in binary = `1023` in decimal). You can try this using the Windows Calculator if you like. Press Alt - 3 for Programmer mode and choose *Bin*. Type ten 1s in and then click *Dec*.
