...
7. Press `Ctrl - O` then Enter to save, followed by `Ctrl - X` to quit from nano.
8. Run the code and remember to the `sudo` command:

  `sudo ./pullup.py`

9. You'll notice the text scrolls up a *lot* faster this time, this is because the loop runs 100 times a second. Use your finger to flip the bucket and you should see at least a few lines of `LOW` scroll up and dissapear. Press `Ctrl - C` to exit your program.
10. Now that the code is running fast enough to detect the bucket tip we also need to write some extra code to count them.  
