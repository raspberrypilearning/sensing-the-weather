...
4. Enter the code below.
  ...
  **Code walkthrough:**
  ...
  |`pin_value = GPIO.input(pin)`| This is defining a variable called `pin_value` and setting it to the result of the `GPIO.input` function. This will go and get the value of the specified GPIO pin number so we also pass the variable `pin` into `GPIO.input` as a *parameter*. The result value will be either `1` if HIGH or `0` if LOW.|
