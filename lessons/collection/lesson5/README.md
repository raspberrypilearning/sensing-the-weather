...
1. Modify your code to match the code below. Note that `lookup_list` should contain the *ADC number* values from your worksheet. Replace the direction letters (N, NNE, NE etc) with the corresponding *ADC number* from your worksheet. Don't forget that each one must be followed by a comma except the last one.
  ...
  def get_direction(adc_value, jitter_margin):
      ...
      for loop_pos in range(len(lookup_list)):
          lookup_list_value = lookup_list[loop_pos]
          
          bottom_end = lookup_list_value - jitter_margin
          top_end = lookup_list_value + jitter_margin
  ...
  **Code walkthrough:**
  ...
  |`lookup_list_value = lookup_list[loop_pos]` | Declares a variable called `lookup_list_value` and sets it to the corresponding value from `lookup_list` at the position (in the list) specified by `loop_pos`, `lookup_list_value` will then contain a recorded ADC value that we want to compare `adc_value` with. So as `loop_pos` increments for each iteration of the loop `lookup_list_value` will contain the next value from the `lookup_list`.|
  | `bottom_end = lookup_list_value - jitter_margin` | To compensate for jitter we need to know if `adc_value` is within an acceptable range either above or blow `lookup_list_value`. This is what `jitter_margin` is for.|
  | `top_end = lookup_list_value + jitter_margin` | .|
1. Press `Ctrl - O` then `Enter` to save, followed by `Ctrl - X` to quit from nano.
