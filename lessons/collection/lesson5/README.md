...
1. Modify your code to match the code below. Note that `lookup_list` should contain the *ADC number* values from your worksheet. Replace the direction letters (N, NNE, NE etc) with the corresponding *ADC number* from your worksheet. Don't forget that each one must be followed by a comma except the last one.
  ...
  **Code walkthrough:**
  ...
  |`test_value = lookup_list[loop_pos]` | Declares a variable called `test_value` and sets it as the corresponding value from `lookup_list` at the list position specified by `loop_pos`, `test_value` will then contain a recorded ADC value that we want to compare `adc_value` with. So as `loop_pos` increments for each iteration of the loop `test_value` will contain the next successive value from the `lookup_list`.|
